        ...
        <div class="ibox-body">
            <div class="row">
                <div class="col-md-12 mb-3 text-right">
                    <div class="col-md-12 mb-3 text-right">
                        <form target="_blank" method="post" action="{{ route('guru.absensi-pdf', ['kelas_id' => $kelas_id]) }}" class="form-inline d-inline">
                            @csrf
                    ...
                    @foreach ($absensi as $index => $a)
                        <tr class="text-center">
                            ...
                            <td>
                                ...
                                <form id="deleteForm{{ $a->id }}" action="{{ route('guru.delete-absensi-perkelas', ['id' => $a->id]) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="modal-dialog modal-dialog-scrollable modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="updateForm{{ $a->id }}" action="{{ route('guru.update-absensi-perkelas', ['id' => $a->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" id="editButton{{ $a->id }}" class="btn btn-primary">Update</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<div class="modal fade tambahAbsen" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('guru.siswa-absensi-store') }}" method="POST">
                    @csrf
...
<script>
    $(document).ready(function() {
    @foreach ($absensi as $a)
        $('#editButton{{ $a->id }}').click(function(e) {
            e.preventDefault();
            Swal.fire({
                title: 'Apakah Anda ingin mengubah absensi?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
